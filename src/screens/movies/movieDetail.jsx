...
const MovieDetail = ({ route }) => {
  ...
  const posterUri = useMemo(() => 
    movieData?.poster_path ? `${IMAGE_URL}${movieData.poster_path}` : null
     ...
     <ScrollView >
      ...
      <View style={{ padding: 20, paddingTop: screenHeight * 0.07 }}>
        <Text style={{ color: Colors.WHITE, fontSize: 24, fontWeight: '700', marginBottom: 10 }}>
          {movieData.title || movieData.name || 'No Title'}
        </Text>
        ...
        <Text style={{ color: '#888', fontSize: 14, marginTop: 4 }}>
          Runtime: {movieData.runtime ? `${movieData.runtime} minutes` : 'N/A'}
        </Text>
